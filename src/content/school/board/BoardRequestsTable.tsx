...
const BoardRequestsTable: FC<BoardRequestsTableProps> = ({ boardRequests }) => {
  ...
  const handleSelectAllBoardRequests = (
    event: ChangeEvent<HTMLInputElement>
  ): void => {
    setSelectedBoardRequests(
  ...
  const selectedSomeVerifyRequests =
    selectedBoardRequests.length > 0 &&
    selectedBoardRequests.length < boardRequests.length;
  ...
  return (
    <>
      <Card>
        ...
        {!selectedBulkActions && <CardHeader title="게시판 요청 목록" />}
        <Divider />
        <TableContainer>
          <Table>
            <TableHead>
            ...
            <TableBody>
              {paginatedBoardRequests.map((boardRequest) => {
                const isBoardRequestSelected = selectedBoardRequests.includes(
                  boardRequest.id
                );
                return (
                  <TableRow
                    ...
                    <TableCell align="right" width={100}>
                      <Tooltip title="승인" arrow>
                        <IconButton
                          ...
                          size="small"
                          onClick={() =>
                            handleBoardRequest(boardRequest.id, Process.success)
                          }
                        >
                      ...
                      <Tooltip title="거부" arrow>
                        <IconButton
                          ...
                          size="small"
                          onClick={() =>
                            handleBoardRequest(boardRequest.id, Process.denied)
                          }
                        >
